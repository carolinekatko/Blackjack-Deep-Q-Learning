...
# Source code altered for tests 0 and 1
...
class DQN(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, hidden_dim1=256, hidden_dim2=256, hidden_dim3=256):
        ...
        self.output = nn.Linear(hidden_dim3, action_dim)
...
class Agent:
    ...
    def run(self, is_training=True, render=False):
        ...
        policy_dqn = DQN(num_states, num_actions, self.fc1_nodes, self.fc2_nodes, self.fc3_nodes).to(device)
        rewards_per_episode = []
        reward_log = []
        ...
        if is_training:
            ...
            target_dqn = DQN(num_states, num_actions, self.fc1_nodes, self.fc2_nodes, self.fc3_nodes).to(device)
        ...
        for episode in itertools.count():
            ...
            while not terminated and episode_reward < self.stop_on_reward:
                if is_training and random.random() < epsilon:
                    ...
                    with torch.no_grad():
                        ...
                        # Debugging: Print Q-values and chosen action
                        #print(f"State: {state}, Q-Values: {q_values.cpu().numpy()}, Chosen Action: {action}")
            ...
            # Calculate win rate over the last 1000 episodes
            if is_training and len(rewards_per_episode) % 1000 == 0:
                win_rate = (win_count / 1000) * 100
                print(f"Episode {episode}: Win Rate (last 1000 episodes): {win_rate:.2f}%")
                win_count = 0  # Reset win counter for the next batch
